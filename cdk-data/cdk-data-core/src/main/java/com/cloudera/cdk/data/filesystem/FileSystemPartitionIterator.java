...
class FileSystemPartitionIterator implements Iterator<CompleteKey>, Iterable<CompleteKey> {
  ...
  private final Iterator<CompleteKey> iterator;
  ...
  /**
   * Predicate to determine whether a {@link com.cloudera.cdk.data.spi.CompleteKey} is in a {@link MarkerRange}.
   */
  private static class InRange implements Predicate<CompleteKey> {
    ...
    @Override
    public boolean apply(CompleteKey key) {
      return range.contains(key);
    }
  }

  /**
   * Conversion function to transform a List into a {@link com.cloudera.cdk.data.spi.CompleteKey}.
   */
  private static class MakeKey implements Function<List<String>, CompleteKey> {
    private final List<FieldPartitioner> partitioners;
    private final CompleteKey reusableKey;
    private final PathConversion convert;

    public MakeKey(PartitionStrategy strategy) {
      this.partitioners = strategy.getFieldPartitioners();
      this.reusableKey = new CompleteKey(strategy);
    ...
    public CompleteKey apply(List<String> dirs) {
  ...
  @Override
  public Iterator<CompleteKey> iterator() {
    return this;
  }
